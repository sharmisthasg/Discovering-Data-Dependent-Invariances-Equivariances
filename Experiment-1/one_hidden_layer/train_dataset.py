...
for hp in hps:
    ...
    for epoch in range(1000):
        ...
        if loss.item() < best_training_loss:
            ...
            torch.save(model.state_dict(), "../models/one_hidden_layer/best_training_" + str(hp) + ".pt")
    print("Best training loss for hp = " + str(hp) + " is " + str(best_training_loss))
    model.load_state_dict(torch.load("../models/one_hidden_layer/best_training_" + str(hp) + ".pt"))
    model.eval()
    with torch.no_grad():
...
model.load_state_dict(torch.load("../models/one_hidden_layer/best_validation.pt"))
