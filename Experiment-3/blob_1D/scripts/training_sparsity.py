...
def custom_loss(criterion, y_pred, y, model):
    hp = 1e-6
...
def train(model,criterion,optimizer,X,y):
    model.train()
    y_pred = model(X)
    loss = custom_loss(criterion,y_pred,y,model)
...
def main():
    ...
    for epoch in range(500):
        ...
        if training_loss < optimum_training_loss and validation_loss < optimum_validation_loss:
            ...
            torch.save(model.state_dict(), "./models/1D_model_sparsity" + ".pt")

    model.load_state_dict(torch.load("./models/1D_model_sparsity" + ".pt"))
