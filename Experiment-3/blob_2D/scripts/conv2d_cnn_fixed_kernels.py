...
class Model2D(torch.nn.Module):
    def __init__(self):
        ...
        #self.conv1.weight[0] = torch.reshape(torch.load('patch1.pt'),(1, 5, 5))
        #self.conv1.weight[1] = torch.reshape(torch.load('patch2.pt'),(1, 5, 5))
        #self.conv1.weight[2] = torch.reshape(torch.load('patch3.pt'),(1, 5, 5))
    def forward(self, x):
...
def test(model,criterion,X,y,validation):
    model.eval()
    with torch.no_grad():
        model.conv1.weight[0] = torch.reshape(torch.load('patch1.pt'),(1, 5, 5))
        model.conv1.weight[1] = torch.reshape(torch.load('patch2.pt'),(1, 5, 5))
        model.conv1.weight[2] = torch.reshape(torch.load('patch3.pt'),(1, 5, 5))
