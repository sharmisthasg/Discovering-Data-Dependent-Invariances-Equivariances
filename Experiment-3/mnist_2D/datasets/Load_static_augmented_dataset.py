...
def main():
	...
	pad = np.random.randint(0, 28, (len(train), 2))
	train_pad = np.zeros((len(train), 4), dtype=int)
	train_pad[:, 0] = pad[:, 0]
	train_pad[:, 1] = 28 - pad[:, 0]
	train_pad[:, 2] = pad[:, 1]
	train_pad[:, 3] = 28 - pad[:, 1]
	train_new = torch.zeros((len(train), 56, 56))
	...
	pad = np.random.randint(0, 28, (len(test), 2))
	test_pad = np.zeros((len(test), 4), dtype=int)
	test_pad[:, 0] = pad[:, 0]
	test_pad[:, 1] = 28 - pad[:, 0]
	test_pad[:, 2] = pad[:, 1]
	test_pad[:, 3] = 28 - pad[:, 1]
	test_new = torch.zeros((len(test), 56, 56))
