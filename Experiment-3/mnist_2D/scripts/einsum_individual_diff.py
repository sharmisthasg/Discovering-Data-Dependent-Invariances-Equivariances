...
def custom_loss_fn(model, criterion, y_pred, y, hp=1e-4):
	...
	# 		curr = next
	# 	total_diff += diff
	# parent = np.random.randint(0, count - 1)
	# for c in range(10):
	# 	weight = weights[c].reshape(count,56,56)
	# 	diff = torch.sum(torch.abs(weight[parent]-weight))
	# 	total_diff += diff
	b = 4
	b_hash = 16
	for c in range(10):
		weight = weights[c]
		rolled_weight = torch.roll(weight, shifts=(b_hash,b_hash,b,b), dims=(0,1,2,3,))
		total_diff += torch.sum(torch.abs(weight - rolled_weight))
...
def main():
	...
	for epoch in range(500):
		...
		if validation_loss < optimum_validation_loss:
			...
			torch.save(model.state_dict(), "../models/2D_mnist_mlp_trial_28_ind" + ".pt")

	model.load_state_dict(torch.load("../models/2D_mnist_mlp_trial_28_ind" + ".pt"))
